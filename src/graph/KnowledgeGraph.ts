import { tokenizeAndStem } from "../shared/tokenizer";
import type { GraphDocument, GraphResult, Intent, KnowledgeDomainNode, KnowledgeGraph } from "./types";
...
const DOMAIN_NODES: KnowledgeDomainNode[] = [
...
function buildDocument(node: KnowledgeDomainNode): GraphDocument {
  const tokens = tokenizeAndStem(node.keywords.join(" "));
...
function computeDocumentFrequency(documents: GraphDocument[]): Map<string, number> {
...
function bm25Score(graph: KnowledgeGraph, document: GraphDocument, queryTerms: string[]): number {
...
function fallbackNodesForIntent(graph: KnowledgeGraph, intent: Intent): GraphResult[] {
  return graph.nodes
...
export function buildGraph(nodes: KnowledgeDomainNode[] = DOMAIN_NODES): KnowledgeGraph {
...
export function queryGraph(graph: KnowledgeGraph, prompt: string, intent: Intent): GraphResult[] {
  const queryTerms = tokenizeAndStem(prompt);
  ...
  const combined = new Map<string, GraphResult>();
...
export function listKnowledgeDomains(): KnowledgeDomainNode[] {
  return structuredClone(DOMAIN_NODES);
}

export const DEFAULT_KNOWLEDGE_GRAPH: KnowledgeGraph = buildGraph();
